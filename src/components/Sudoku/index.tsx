...
const Sudoku: React.FC<Props> = () => {
  const getGivenNumberCell = useCallback(
    (value: number) => <td className={`${classes.cell} ${classes.givenNumber} givenCell`}>{value}</td>,
    []
  );
    ...
    <ProjectPage background={PROJECT_COLORS.PROJECT36.background} containerClassName={classes.pageContainer}>
      <QuestionDetails
        ...
        <p>
          ...
          <a href='https://www.nytimes.com/puzzles/sudoku'>The New York Times Number Puzzles</a>, and you need to make
          the game functional using JavaScript.
        </p>
        <p>
          Sudoku is a popular puzzle game, where the goal is to fill in every cell of a 9x9 grid with the numbers one
          through nine such that no duplicates appear in any row, column, or 3x3 subgrid.
        </p>
        <p>The provided board contains two types of cells: given numbers and standard cells:</p>
        <ul>
          <li>
            Given numbers have the <span className={classes.questionDescriptionHighlight}>given-number</span> class, are
            grey with a pre-filled number, and should have no functionality.
          </li>
          <li>
            Standard cells have two child spans with classes of{' '}
            <span className={classes.questionDescriptionHighlight}>candidates</span> and{' '}
            <span className={classes.questionDescriptionHighlight}>value</span>, respectively, and they're white. When a
            standard cell is clicked on, it should become selected (i.e., the cell should have the{' '}
            <span className={classes.questionDescriptionHighlight}>selected</span> class). Only one cell should be
            selectable at a time (i.e., selecting a cell should unselect the previously selected cell).
          </li>
        </ul>
        <p>
          To the right of the Sudoku board is a provided set of controls. These controls include buttons with the
          numbers one through nine as well as a toggle button (which is an HTML checkbox input field). The controls
          should have the following functionality:
        </p>
        <ul>
          <li>
            When the <span className={classes.questionDescriptionHighlight}>#candidate-switch</span> checkbox is
            unchecked (i.e., set to "Normal Move"), clicking on any of the number control buttons should place that
            number in the <span className={classes.questionDescriptionHighlight}>.value</span> span of the selected
            cell.
          </li>
          <li>
            The <span className={classes.questionDescriptionHighlight}>.candidates</span> spans of standard cells should
            act as lists of candidate moves (potential moves) for each cell. When the{' '}
            <span className={classes.questionDescriptionHighlight}>#candidate-switch</span> checkbox is checked (i.e.,
            set to "Candidate Move"), clicking on any of the number control buttons should toggle that number in the{' '}
            <span className={classes.questionDescriptionHighlight}>.candidates</span> span of the selected cell. In
            other words, if the <span className={classes.questionDescriptionHighlight}>.candidates</span> span already
            contained the number, it should be removed; otherwise it should be added.
          </li>
          <li>
            The contents of the <span className={classes.questionDescriptionHighlight}>.candidates</span> spans should
            always be sorted in ascending order, and there shouldn't be duplicate numbers in an individual candidates
            list. There should also be no spaces or other delimiters between numbers in a candidates list.
          </li>
        ...
        <div className={classes.sudokuContainer}>
          ...
          <div className={classes.controls}>
            ...
            <Input
              id='switch'
              type='checkbox'
              className={classes.candidateSwitch}
